...
start = time.time()
DEFAULT_URL = ("https://www.glassdoor.com/Reviews/Aon-Reviews-E53.htm")
...
parser.add_argument('-l', '--limit', default=100000000,
...
def scrape(field, review, author):
    ...
    def scrape_recommendation(review):
        try:
            s = review.find_elements_by_xpath(
                ".//div[@class='row reviewBodyCell recommends']/div")
...
def more_pages():
    try:
        browser.find_element_by_class_name('pagination__PaginationStyle__current')
        return True
    except selenium.common.exceptions.NoSuchElementException:
        return False
...
def check_error():
    try:
        browser.find_element_by_xpath(".//div[@class='cf-error-details cf-error-504']")
        return True
    except selenium.common.exceptions.NoSuchElementException:
        return False
...
def navigate_to_reviews(url):
    ...
    reviews_cell = browser.find_element_by_xpath(
        "//*[@id='EmpLinksWrapper']/div//a[@class='eiCell cell reviews active']"
        )
...
def get_browser():
    ...
    browser = wd.Chrome(executable_path='/usr/local/bin/chromedriver',options=chrome_options)
    return browser
...
def get_current_page():
    ...
    current_ = paging_control.find_element_by_css_selector('a')
    current = int(current_.text)
    return current
...
def main():
    ...
    for x in range(len(links)):
        ...
        count = 0
        fileName = args.url.rsplit('/', 1)[-1].rpartition("-")[0]
        args.file = f'Review/{fileName}.csv'

        if not args.start_from_url:
            reviews_exist = navigate_to_reviews(args.url)
        ...
        logger.info(f'Added {len(res)} reviews to {args.file}.')

        while more_pages():
            if last_page():
                pass
            else:
                next = browser.find_element_by_xpath(".//li[@class='pagination__PaginationStyle__next']/a")
            ...
            res = res.append(reviews_df)
            count += len(res)
            res.to_csv(args.file, mode='a', index=False, encoding='utf-8', header=False)
            logger.info(f'Added {len(res)} more reviews to {args.file}.')

        logger.info(f'Finished reviewing {args.url} with {count} reviews')
